...
const errorNombre = computed(() => {
  const v = contacto.nombre.trim();
  if (!v) return '';
  if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(v)) return 'El nombre solo debe contener letras y espacios.';
  if (v.length < 2) return 'El nombre debe tener al menos 2 caracteres.';
  return '';
});

const errorTelefono = computed(() => {
  const v = contacto.telefono.trim();
  if (!v) return '';
  if (!/^\+?[0-9\s\-]+$/.test(v)) return 'El teléfono solo debe contener números.';
  const soloNumeros = v.replace(/\D/g, '');
  if (soloNumeros.length < 7 || soloNumeros.length > 15) return 'El teléfono debe tener entre 7 y 15 dígitos.';
  return '';
});
...
const formularioValido = computed(() => {
  return (
    ...
    !errorNombre.value &&
    contacto.telefono.trim().length >= 7 &&
    !errorTelefono.value
  );
});
...
const stockMaximo=(item)=>productoVisual(item)?.stock ?? item.stock ?? Infinity;
...
<div v-if="items.length" class="checkout-grid"><section><h2>Productos seleccionados <small>({{cantidadItems}})</small></h2><div class="items"><article v-for="item in items" :key="item.productoId"><div class="item-visual"><ProductoVisual :producto="productoVisual(item)"/></div><div class="item-info"><small>{{item.categoria}}</small><h3>{{item.nombre}}</h3><p>{{item.marca}} · <span v-if="stockMaximo(item) !== Infinity">Stock: {{stockMaximo(item)}}</span></p><button @click="carrito.quitarProducto(item.productoId)">Eliminar</button></div><div class="item-precio"><strong>${{item.precioUnitario.toFixed(2)}}</strong><div><button @click="carrito.cambiarCantidad(item.productoId,item.cantidad-1)">−</button><span>{{item.cantidad}}</span><button :disabled="item.cantidad >= stockMaximo(item)" :title="item.cantidad >= stockMaximo(item) ? 'Stock máximo alcanzado' : ''" @click="carrito.cambiarCantidad(item.productoId,item.cantidad+1)">+</button></div></div></article></div><router-link to="/tienda" class="seguir">← Seguir seleccionando</router-link></section>
<aside><div class="resumen"><h2>Resumen referencial</h2><p><span>Productos</span><strong>{{cantidadItems}}</strong></p><p class="total"><span>Total estimado</span><strong>${{total.toFixed(2)}}</strong></p><small>El negocio confirmará stock, entrega y precio final por WhatsApp.</small></div><form @submit.prevent><h3>Datos de contacto</h3><label>Nombre<input v-model="contacto.nombre" placeholder="Ej. Juan Pérez" required><small v-if="errorNombre" class="error-msg">{{errorNombre}}</small></label><label>Teléfono<input v-model="contacto.telefono" type="tel" placeholder="Ej. 70012345" required><small v-if="errorTelefono" class="error-msg">{{errorTelefono}}</small></label><label>Nota opcional<textarea v-model="contacto.nota" rows="3" placeholder="Instrucciones adicionales..."></textarea></label><a :class="['whatsapp',{disabled:!formularioValido}]" :href="formularioValido?enlace:null" target="_blank" rel="noopener"><span class="material-symbols-outlined">chat</span>Enviar pedido por WhatsApp</a></form></aside></div>

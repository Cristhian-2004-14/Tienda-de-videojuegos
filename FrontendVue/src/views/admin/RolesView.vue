...
async function guardar(){error.value='';if(!form.permisos.length){error.value='Selecciona al menos un módulo permitido.';return}try{if(editando.value)await rolesApi.actualizar(editando.value,{...form,id:editando.value});else await rolesApi.crear({...form,permisos:[...form.permisos]});limpiar();await cargar()}catch(e){error.value=e.message||'No se pudo guardar el rol.'}}
onMounted(cargar);
</script>
<template><AdminLayout titulo="Roles y permisos"><template #header><AdminPageHeader eyebrow="SEGURIDAD / ACCESO" title="Roles y permisos" description="Los permisos controlan las rutas y opciones visibles de cada usuario."><button class="btn-primary" @click="abierto=!abierto">Nuevo rol</button></AdminPageHeader></template>
<form v-if="abierto" class="panel-caso formulario-caso" @submit.prevent="guardar"><div class="campo"><label>Nombre</label><input v-model.trim="form.nombre" required minlength="2" maxlength="60"></div><div class="campo"><label>Descripción</label><input v-model.trim="form.descripcion" required minlength="5" maxlength="250"></div><fieldset><legend>Módulos permitidos</legend><label v-for="m in modulos" :key="m" class="check"><input v-model="form.permisos" type="checkbox" :value="m">{{m}}</label></fieldset><div class="acciones"><button class="btn-primary">{{editando?'Actualizar rol':'Guardar rol'}}</button><button class="btn-secondary" type="button" @click="limpiar">Cancelar</button></div></form><p v-if="error" class="error" role="alert">{{error}}</p>

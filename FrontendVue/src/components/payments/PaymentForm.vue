...
function submit(){
  ...
  if(!Number.isFinite(monto)||monto<=0||monto>props.max)return;
  emit('submit',{...form,monto});
}
</script>
<template><form class="panel-caso formulario-caso" @submit.prevent="submit"><div class="methods"><label v-for="method in ['Efectivo','QR','Tarjeta','Transferencia']" :key="method" :class="{selected:form.metodoPago===method}"><input v-model="form.metodoPago" type="radio" :value="method" required><span class="material-symbols-outlined">{{method==='Efectivo'?'payments':method==='QR'?'qr_code_2':method==='Tarjeta'?'credit_card':'account_balance'}}</span>{{method}}</label></div><div class="campo"><label>Monto a registrar</label><input v-model.number="form.monto" type="number" min=".01" :max="max" step=".01" required></div><div class="campo"><label>Referencia o nota (opcional)</label><input v-model.trim="form.referencia" maxlength="120" placeholder="Ej. comprobante 88421"></div><button class="btn-primary" :disabled="loading">{{loading?'Registrando...':submitText}}</button></form></template>
